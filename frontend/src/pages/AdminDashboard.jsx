...
import { LayoutDashboard, Newspaper, Image, MessageSquare, Users, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-100 min-h-screen flex flex-col lg:flex-row">
        ...
        <nav className="p-4 flex flex-row lg:flex-col gap-1 overflow-x-auto">
          {NAV_ITEMS.map(({ tab, label, icon }) => (
            <button key={tab} onClick={() => setCurrentTab(tab)} 
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-xs font-bold uppercase transition-all w-full ${currentTab === tab ? 'bg-amber-500 text-[#0c2340]' : 'hover:bg-slate-800'}`}>
              {icon} {label}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 p-6 lg:p-8 overflow-y-auto relative">
        {isLoading && <div className="absolute top-4 right-8 bg-amber-500 text-[#0c2340] px-3 py-1 rounded text-[10px] font-black animate-pulse">Syncing...</div>}
        ...
        {/* FIXED: Passed the explicit arrow function to onDelete */}
