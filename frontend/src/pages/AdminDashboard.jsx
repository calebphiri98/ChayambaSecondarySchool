...
import { LayoutDashboard, Newspaper, Image, MessageSquare, Users, Loader2, LogOut, KeyRound } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (currentTab === 'reset-password') return;
    fetchTabClusterData();
  }, [currentTab]);
  ...
  const NAV_ITEMS = [
    ...
    { tab: 'reset-password', label: 'Reset Password', icon: <KeyRound size={16} /> },
  ];
  ...
  return (
    <div className="bg-slate-100 min-h-screen flex flex-col lg:flex-row">
      ...
      <main className="flex-1 p-6 lg:p-8 overflow-y-auto relative">
        ...
        {currentTab === 'reset-password' && <AdminResetPassword />}
      </main>
    </div>
  );
}
